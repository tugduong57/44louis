...
# 1. Xác định đường dẫn gốc (thư mục chứa file main.py)
BASE_DIR = Path(__file__).resolve().parent

# 2. Xây dựng đường dẫn đến thư mục bin của ffmpeg
ffmpeg_bin_path = BASE_DIR / "resourse4whisper" / "ffmpeg-8.0.1-essentials_build" / "bin"

# 3. Thêm vào biến môi trường PATH
# Chúng ta đưa nó lên đầu danh sách để hệ thống ưu tiên tìm ở đây trước
os.environ["PATH"] = str(ffmpeg_bin_path) + os.pathsep + os.environ["PATH"]

# 4. Kiểm tra thử xem Python đã "thấy" ffmpeg chưa
ffmpeg_exe = shutil.which("ffmpeg")
if ffmpeg_exe:
    print(f"✅ Đã kết nối FFmpeg thành công tại: {ffmpeg_exe}")
else:
    print(f"❌ Không tìm thấy FFmpeg tại: {ffmpeg_bin_path}")
    print("Vui lòng kiểm tra lại tên thư mục (ví dụ: 'resourse' hay 'resource')")
...
if __name__ == "__main__":
    ...
    MY_AUDIO = r"How to Talk to Anyone with Confidence  English Podcast For Learning English  English Leap Podcast.mp3"
